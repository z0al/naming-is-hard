...
This proposal is still in a very early stage and should be considered as _draft_. Before reaching version 1.0, changes may be introduced without notice.
...
- `link` (string) is the URL of the resource that the feed describes. This resource may or may not actually be a “home” page, but it should be an HTML page.
...
- `link` (string) is the URL of the resource described by the story.
...
- `until` (string) specifies the expiry date in [ISO 8601][4] format in case the story is expirable. When a story is expired, the reader software should assume that the URL(s) in `url` are no longer accessible.
