...
- `alternates` (array) an array of strings specifies alternatives URLs for the same story in a different platform. For example, a publisher may republish some of their notes (originally on their website, as indicated by `link`) to Twitter, in this case, they include a link of that specific tweet so people can reach and engage with them.
